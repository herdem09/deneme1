...
# Render.com'dan veri almak
def veri_al_render_com():
    url = "http://render.com/api/durum"  # Render.com'daki API'yi doğru URL ile değiştirin
    response = requests.get(url)

    if response.status_code == 200:
        return response.json()  # Veriyi JSON formatında döner
    else:
        return None
...
# Render.com'dan veri alıp sayfada gösterme
@app.route('/durum')
def durum():
    render_com_durum = veri_al_render_com()
    if render_com_durum:
        return render_template('durum.html', durum=render_com_durum)
    else:
        return "Veri alınamadı", 500
